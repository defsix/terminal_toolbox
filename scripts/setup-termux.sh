...
# (confirmed live on a real device: exact right theme name in config.toml,
# still the wrong colors). "catppuccin" here specifically means the
# Macchiato flavor (see the oh-my-posh theme block above), matching
# Superfile's "catppuccin-macchiato" exactly. Only themes with no bundled
# Superfile equivalent (m365princess, atomic, jandedobbeleer, marcduiker,
# neko) fall back to our own generated file, prefixed so it can never
# collide with any bundled name, present or future.
case "$THEME" in
...
# for the terminals this repo targets (Termux's own terminal app, and
# virtually every modern terminal emulator, genuinely support truecolor).
